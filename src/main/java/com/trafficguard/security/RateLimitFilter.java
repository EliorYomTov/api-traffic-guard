...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    public RateLimitFilter(RateLimitService rateLimitService, ObjectMapper objectMapper, SecurityEventService securityEventService, IpBlockService ipBlockService) {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        ...
        // Check IP block before rate limiting
        ...
        if (!rateLimitService.isAllowed(clientId)) {
            ...
            response.setContentType(MediaType.APPLICATION_JSON_VALUE);
            objectMapper.writeValue(response.getWriter(), Map.of("status", 429, "error", "Too Many Requests", "message", "Rate limit exceeded. Try again in " + rateLimitService.getWindowSeconds() + " seconds."));
            return;
        }
    ...
    /**
     * Client identity: authenticated username > X-Forwarded-For > remote IP
     */
